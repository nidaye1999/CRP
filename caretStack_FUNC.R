...
### a function to convert categorical predictors to dmuuy coding ###
...
plot.perf <- function(model.perf, metrics='all'){
  ...
  if (metrics=='all'){
    ...
    p <- ggplot(data = tmp, aes(x = method, y = m, fill = method)) + 
      geom_bar(stat = "identity", position = position_dodge(0.9)) +
      geom_errorbar(limits, position = position_dodge(0.9), width = 0.25) +
...
# Wrap-up function for repeated, nested CV
rNCV.old <- function(data, resp.var, ref.lv=NULL, nRep, nFolds.outer, ML.methods, 
  ...
  res <- foreach(r.out=1:nRep, .combine=comb_rep, .packages='caret') %dopar% {
    index.outer <- createFolds(data[, resp.var], k=nFolds.outer, list=F)
    
    res.outer <- foreach(k.outer=1:nFolds.outer, .combine=comb_outer, .packages='caret') %dopar% {
      ...
      save(models, 
           file = paste0(dir.path, resp.var, '_', file.root, '_Rep_', r.out, '_fold_', k.outer, '.rda'))
      ...
      y.pred <- matrix(NA, nrow(data), length(resp.lv))
      y.pred[index.outer==k.outer, ] <- 
        ifelse(length(ML.methods)==1, 
               pred.val$prediction$test[[1]][, resp.lv], 
               pred.val$prediction$test$Stack[, resp.lv])
      ...
      perf.by.fold <- data.frame(Rep = r.out, fold = k.outer, perf.v.tmp)
      
      # Step 4. Variable importance
      if (length(ML.methods)==1){
        var.imp <- varImp(models[[1]])
      }
      else {
        var.imp <- data.frame(
          Rep = r.out,
      ...
      # prepare outputs as a list
      if (is.null(stack.wt)){
        stack.wt.tmp <- data.frame(Rep = r.out, fold = k.outer, t(pred.val$weight))
        list1 <- list(stack.wt = stack.wt.tmp, stack.model = pred.val$stack.model,
                      y.pred = y.pred, perf.by.fold = perf.by.fold, var.imp = var.imp)
      } 
      else {
        list1 <- list(stack.wt = stack.wt, stack.model = pred.val$stack.model, 
                      y.pred = y.pred, perf.by.fold = perf.by.fold, var.imp = var.imp)
      }
      return(list1)
    }
    
    y.pred.comb <- apply(res.outer$y.pred, 1, mean, na.rm=T)
    df.comb <- data.frame(obs = data[, resp.var], pred = y.pred.comb)
    perf.comb <- control$summaryFunction(df.comb)
    
    return(list(index.outer    = index.outer,            
                stack.wt    = res.outer$stack.wt,        stack.model = res.outer$stack.model,
                y.pred.by.fold = res.outer$y.pred,       y.pred.comb = y.pred.comb,
                perf.by.fold   = res.outer$perf.by.fold, perf.comb   = perf.comb,
                var.imp        = res.outer$var.imp))
  }
...
varImp_rNCV <- function(rncv.obj){
  ...
  par(mar=c(5,8,4,1)+.1) # need to change
